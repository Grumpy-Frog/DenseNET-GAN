...
from torchmetrics.image import StructuralSimilarityIndexMeasure,PeakSignalNoiseRatio,ErrorRelativeGlobalDimensionlessSynthesis
...
def show_plot():
    for var_name in metrices.keys():
        if var_name != 'epochs':
            var_value = np.array(metrices[var_name])
            plt.plot(metrices['epochs'],var_value, linestyle='-', color='orange')
            plt.xlabel('epochs')
            y_label = var_name[:-5]
            plt.ylabel(y_label)
            plt.title(var_name)
            ...
            # Display the plot
            #plt.show()
            plt.savefig(var_name+'_plot.png')  
            plt.clf()
...
def save_some_examples(gen, val_loader, epoch, folder):
    ...
    with torch.no_grad():
        ...
        # some evaluations
        fretchet_dist = float(calculate_fretchet(y, y_fake, gen))
        ssim = float(calculate_SSIM(y, y_fake))
        psnr = float(calculate_PSNR(y, y_fake))
        ergas = float(calculate_ergas(y, y_fake))
        #mse = calculate_mse(y, y_fake)

        metrices['epochs'].append(int(epoch))
        metrices['fid_list'].append(fretchet_dist)
        metrices['ssim_list'].append(ssim)
        metrices['psnr_list'].append(psnr)
        metrices['ergas_list'].append(ergas)
        show_plot()
